...
declare function tei2:annotate($nodes as node()*, $annotations as element()*, $layer as xs:string, $format as xs:string) {
    ...
    (:TODO: Improve to make it possible for the whole text node to be wrapped up in an (inline) element.:)
    let $target-text := 
        if ($meshed-a8ns/text())
...
declare function tei2:tei2target($node as node()*, $target as xs:string) {
        (:If the element has a text node, separate the text node.:)
        (:TODO: Improve to make it possible for the whole text node to be wrapped up in an (inline) element.:)
        element {node-name($node)}{$node/@*,tei2:separate-text-layers($node, $target)}
